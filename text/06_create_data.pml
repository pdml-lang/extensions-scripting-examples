[ch [title Creating Text And Data]
    ...
    Suppose we want to display compound interest tables like the following one in a [!get PML_link] document.
    [div (html_style="margin-left:3em;")
        [!ins-file path=../../[!get examples_dir]interest_table/example.pml]
    ]

    The above table could be created with the following PML code:
    [insert_code file=[!get examples_dir]interest_table/example.pml]
    ...
    As seen already, functions are defined within a [c !def] node.
    ...
    [insert_code file=[!get examples_dir]interest_table/interestUtils.def]
    ...
    If we want to insert more tables with different parameters, we just need to insert more [c !exp] nodes.
