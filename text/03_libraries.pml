[ch (id=libraries) [title Libraries]
    ...
    [ch [title Standard Javascript Library]
        ...
        [!exp priceExampleSnippet ( "toFixed" )]
    ...
    [ch [title Standard PDML Library]
        ...
        Instead of using a standard Javascript function we can use a [i standard PDML function] that allows us to provide a specific format, and also rounds to the number of decimal places specified in the format. We'll use function [link url=[!get PDML_ext_ref_manual_url]#numberUtils-formatFloat text=formatFloat] defined in object [link url=[!get PDML_ext_ref_manual_url]#numberUtils text=numberUtils]:
        [!exp priceExampleSnippet ( "formatFloat" )]
    ...
    [ch [title User-Defined Library]
        ...
        We can therefore [i define a function] in a [c !def] node:
        [insert_code file=[!get examples_dir]price/resources/libraries/myPriceLibrary.def]
        Now we can use function [c roundPrice] again and again in [c price] nodes. For example:
        [!exp priceExampleSnippet ( "2prices" )]
        ... which expands to:
        [code
            [price 2,675.19]
            ...
    ...
    [ch [title Shared Library]
        ...
        If we need the function in other documents too, we can save the above [c !def] node in a file:
        [caption File resources/libraries/myPriceLibrary.def]
        [insert_code file=[!get examples_dir]price/resources/libraries/myPriceLibrary.def]

        Then we can import the library at the beginning of the document with an [c !ins-file] node (using a relative file path), and use the function as before:
        [!exp priceExampleSnippet ( "final" )]
    ...
    For more information about libraries please refer to [link url=[!get PDML_ext_user_manual_url]#javascript_support text="Javascript Support"].
]
